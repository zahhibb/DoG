...
public class TankGameController : MonoBehaviour {
	...
	void Start () {
        ...
        */
        ...
        {
            GameObject player = (GameObject)Instantiate(m_tank, new Vector3(transform.position.x + (i * 7f), 1f, 0f), Quaternion.Euler(0, 0, 0));

            player.GetComponent<MovementController>().TriggerAxis = m_playerManagers[i].Inputs[16].name;
            player.GetComponent<MovementController>().LeftStickX = m_playerManagers[i].Inputs[12].name;
